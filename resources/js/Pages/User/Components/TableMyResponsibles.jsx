...
const TableMyResponsibles = ({datos}) => {
    const {post} = useForm();
    ...
    const HandleDeleteResponsible = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                post(`/area/remover-responsable/${id}`, {
                    onSuccess: () => {
                        Swal.fire({
                            color: "#d6d6d6",
                            background: "#1f2937",
        ...
        <>
            <div className="md:overflow-x-visible overflow-x-auto">
                <table className="w-full rounded-lg">
                    ...
                    <tr>
                    <th className="px-3 w-max">#</th>
                        <th className="px-3 text-left py-2 ">NOMBRE</th>
                        <th className="px-3 text-left py-2 ">EQUIPO RESPONSABLES</th>
                        <th className="px-3 text-left py-2 ">FECHA DE REGISTRO</th>
                        <th className="px-3 text-left py-2 "></th>
                    </tr>
                    </thead>
                    <tbody className="bg-gray-700 text-gray-300">
                        {datos.length > 0 ? datos.map((datos, index) => (
                            <tr className="text-left font-light text-sm" key={index}>
                                    ...
                                    <Dropdown>
                                        ...
                                        <Dropdown.Content width={"max"} contentClasses={"bg-neutral-900 py-2 z-100"} align={"ltr:origin-top-right rtl:origin-left-left -top-2 right-24"}>
                                            <div className="flex flex-col space-y-2 text-gray-500 select-none" >
                                                <div onClick={() => HandleOpenModal(datos.id)} className={"cursor-pointer"}>
                                                    <Item
                                                        text={"Editar"}
                                                ...
                                                <div onClick={() => HandleDeleteResponsible(datos.id)} className={"cursor-pointer"}>
                                                    <Item
                                                        text={"Eliminar"}
            ...
            <ResponsibleModalUpdateForm showModal={showModal} setCloseModal={HandleCloseModal} id={idSelect}/>
        </>
    );
}
export default TableMyResponsibles;
