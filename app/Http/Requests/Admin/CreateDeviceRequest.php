...
class CreateDeviceRequest extends FormRequest
{
    ...
    public function messages(): array
    {
        return [
            //CAMPOS COMUNES
            'area_id.required' => 'Área: Seleccione un area',
     ...
     * @throws Exception
     */
    public function update(CreateDeviceRequest $updateDeviceRequest, $device): void
    {
        try{
            DB::beginTransaction();

            if($device){
                if($updateDeviceRequest->responsible_id !== $device->responsible_id){
                    $responsible = (new ResponsibleServices())->RegisterOrGetResponsibleWithAreaId($updateDeviceRequest);
                    $device->responsible_id = $responsible->id;
                }

                $device->serial_number = $updateDeviceRequest->serial_number;
                $device->condition = $updateDeviceRequest->condition;
                $device->acquisition = $updateDeviceRequest->acquisition;
                $device->warranty = $updateDeviceRequest->warranty;
                $device->comments = $updateDeviceRequest->comments;

                (new DevicesUpdateService())
                    ->UpdateDetailsDeviceByType($device->types->name, $updateDeviceRequest, $device->id);
    ...
    {
        $responsible = (new ResponsibleServices())->RegisterOrGetResponsibleWithAreaId($request);
